...
newjoin = jp.join(refs,jp.PaperId == refs.PaperId)
newNames = [ 'JournalId','PaperId2', 'PaperTitle','CitationCount','Year','PaperId','PaperReferenceId']
newjoin2 = newjoin.toDF(*newNames)


#get journal info for references cited by papers 

papers =  MAG.getDataframe( 'Papers' )
refJournals = papers.join( newjoin2 , papers.PaperId == newjoin2.PaperId)\
                   .select(newjoin2.PaperId,newjoin2.PaperReferenceId,papers.JournalId)


#newNames = [ 'PaperId','Rank', 'Doi','DocType','PaperTitle','OriginalTitle','BookTitle','Year','Date','OnlineDate','Publisher','JournalId','ConferenceSeriesId','ConferenceInstanceId',"Volume",'Issue','FirstPage','LastPage',"ReferenceCount",'CitationCount','EstimatedCitation','OriginalVenue','Family']
...
#write output
refJournals.write.csv("PapersReferencesJournals")



#get journal ISSNs

journalISSN = journal.select(journal.JournalId , 
                                        journal.NormalizedName, 
                                        journal.Issn)
journalISSN.write.csv("journalISSN")
